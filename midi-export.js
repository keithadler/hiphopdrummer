...
function exportMIDI(opts) {
  ...
  // Chord sheet text file
  if (opts.chordSheet && _lastChosenKey && _lastChosenKey.i) {
    var cs = [];
    cs.push('HIP HOP DRUMMER — Chord Sheet');
    cs.push('==============================');
    cs.push('');
    cs.push('BPM: ' + bpm + '  |  Key: ' + _lastChosenKey.root);
    cs.push('I = ' + _lastChosenKey.i + '  |  IV = ' + _lastChosenKey.iv + '  |  V = ' + _lastChosenKey.v);
    cs.push('');
    cs.push('PROGRESSION BY SECTION');
    cs.push('----------------------');
    var csExported = {};
    arrangement.forEach(function(sec) {
      if (csExported[sec]) return;
      csExported[sec] = true;
      var bars = Math.ceil((secSteps[sec] || 32) / 16);
      var line = (SL[sec] || sec) + ':  ';
      for (var b = 0; b < bars; b++) {
        var barInPhrase = b % 4;
        if (barInPhrase === 2) line += '| ' + _lastChosenKey.iv + ' ';
        else if (barInPhrase === 3 && bars > 2) line += '| ' + _lastChosenKey.v + ' ';
        else line += '| ' + _lastChosenKey.i + ' ';
      }
      line += '|';
      cs.push(line);
    });
    ...
    cs.push('NOTES');
    cs.push('-----');
    cs.push('I (home) = ' + _lastChosenKey.i + ' — start and end here');
    cs.push('IV (tension) = ' + _lastChosenKey.iv + ' — movement away from home');
    cs.push('V (resolution) = ' + _lastChosenKey.v + ' — pulls back to the I');
    cs.push('');
    cs.push('Bass follows this same progression (root notes only).');
    cs.push('Chords change every 2 bars in a 4-bar phrase.');
    cs.push('');
    cs.push('Generated by Hip Hop Drummer — keithadler.github.io/hiphopdrummer');
    folder.file('chord_sheet_' + _lastChosenKey.root.replace(/[#\/]/g, '') + '.txt', cs.join('\r\n'));
  }
